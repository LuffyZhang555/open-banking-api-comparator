import React from 'react';
import ReactJson from 'react-json-view';

import { useRequest } from 'umi';
import hsbcApi from '@/services/hsbc';
import hengshengApi from '@/services/hengsheng';
import { Descriptions,Radio,Space,Popconfirm,Button,Card,Badge,Image } from 'antd';

import { PageContainer } from '@ant-design/pro-layout';



export interface ProductProps {
 
  type?: string;
  default?: string;
}

const BankProductsCompare: React.FC<ProductProps> = (props: any) => {

  
  const { data, loading } = useRequest<any>(
    () => {
      switch (props.type) {
        case 'hsbc,allInOne':
          return hsbcApi.GetAllInOneAccount();
        case 'hsbc,business':
          return hsbcApi.GetBusinessIntegratedAccounts();
        case 'hengsheng,current':
          return hengshengApi.GetCurrentAccount();
        default:
          return hengshengApi.GetSavingAccount();
      }
    },
    {
      formatResult: (res) => {
        return res.data[0];
      },
    },
  );
  if (loading) {
    return <>Loading...</>;
  }

  return (
   
    <>
      
      <h1> {data.Brand[0].BrandName}   </h1>
      
      <Image width={200} src="data:image/png;base64," />
         
      
      
      
      <Card>
        
      <Descriptions title={data.Brand[0].SavingsAccount[0].Name} bordered>
       <Descriptions.Item label="Brand Name" span={1}>{data.Brand[0].BrandName} </Descriptions.Item>
       <Descriptions.Item label="Product Name" span={2}>{data.Brand[0].SavingsAccount[0].Name}</Descriptions.Item>
       

       <Descriptions.Item label="Features"span={3}>
         *{data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].FeaturesAndBenefits.FeatureBenefitItem[0].Name}
         <br />
         *{data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].FeaturesAndBenefits.FeatureBenefitItem[1].Name}
         <br />
         *{data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].FeaturesAndBenefits.FeatureBenefitItem[2].Name}:
         <br />
         
       </Descriptions.Item>

       <Descriptions.Item label="Fee Charges" span={3}>
         *{data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].FeesCharges.FeeChargeDetail[0].FeeName}:{data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].FeesCharges.FeeChargeDetail[0].Notes[0]}
         <br />
         *{data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].FeesCharges.FeeChargeDetail[1].FeeName}:{data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].FeesCharges.FeeChargeDetail[1].FeeAmount} {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].FeesCharges.FeeChargeDetail[1].FeeAmountCurrencyCode}
         <br />
         *{data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].FeesCharges.FeeChargeDetail[2].FeeName}:{data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].FeesCharges.FeeChargeDetail[2].FeeAmount} {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].FeesCharges.FeeChargeDetail[2].FeeAmountCurrencyCode}
       </Descriptions.Item>



       <Descriptions.Item label="Credit Interest" span={3}>
         * If the account balance is between 0  and  {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].CreditInterest.TierBand[0].TierValueMaximum}, the interest rate will be {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].CreditInterest.TierBand[0].InterestRate} (HKD)
          <br />
         * If the account balance is between {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].CreditInterest.TierBand[1].TierValueMinimum}  and  {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].CreditInterest.TierBand[1].TierValueMaximum}, the interest rate will be {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].CreditInterest.TierBand[1].InterestRate} (HKD)
          <br />
         * If the account balance is between {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].CreditInterest.TierBand[2].TierValueMinimum}  and  {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].CreditInterest.TierBand[2].TierValueMaximum}, the interest rate will be {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].CreditInterest.TierBand[2].InterestRate} (HKD)
          <br />
         * If the account balance is between {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].CreditInterest.TierBand[3].TierValueMinimum}  and  {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].CreditInterest.TierBand[3].TierValueMaximum}, the interest rate will be {data.Brand[0].SavingsAccount[0].SavingsAccountMarketingState[0].CreditInterest.TierBand[3].InterestRate} (HKD)
          <br />
       </Descriptions.Item>


       <Descriptions.Item label="Notes" span={3}>
         * see other detail at https://www.hangseng.com/en-hk/personal/banking-services/accounts-and-deposits/
         <br/>
         * HotlineNumber: (852) 2822 0228
       </Descriptions.Item>


    
      </Descriptions>

      <br />
          <Space>
           <Button type="primary"
           href="https://www.hangseng.com/en-hk/personal/banking-services/accounts-and-deposits/">
          click me to see more
           </Button>
           <Popconfirm title="Are you sure to add this one?" okText="Yes" cancelText="No">
           <Button>Add to my favorate</Button>
           </Popconfirm>
           </Space>
 

     </Card>
      
      


    

     
     




     




    



    </> 


  );

};
  
export default BankProductsCompare;




